...
public class Listeners extends BaseClass implements ITestListener //we are calling here the extend report utility
{
	...
	ExtentReports extent = ExtendReportUtility.createExtentReports();//calling create extend reports from the ExtendReport utility   	
	ThreadLocal<ExtentTest> extentTest = new ThreadLocal<ExtentTest>();//for ensure thread safety for parallel testing/running
	...
	public void onTestFailedButWithinSuccessPercentage(ITestResult result) //call when test is failed but meeting the defined success percentage
	{ 
